    ...
    public function sessionDataFilter($sessionData) {
        ...
        if ($authenticator->connect()) {
            if ($authenticator->login($_POST["user"], $_POST["password"])) {
                ...
                // default values if the fields are not filled in LDAP
                $lastname = get_translation("lastname");
                $firstname = get_translation("firstname");
                ...
                if ($userData) {
                    ...
                    $firstname = !empty($userData[$fieldMapping["firstname"]][0]) ?
                            $userData[$fieldMapping["firstname"]][0] :
                            $firstname;
                    $lastname = !empty($userData[$fieldMapping["lastname"]][0]) ?
                            $userData[$fieldMapping["lastname"]][0] : $lastname;
                    $email = !empty($userData[$fieldMapping["email"]][0]) ?
                            $userData[$fieldMapping["email"]][0] : $email;
                }
